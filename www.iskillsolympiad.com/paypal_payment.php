...
//echo 'Total Amount:'. $c * $amount."<br>";
 $amount2 = floatval($c) * $amount1;
...
<?php 
error_reporting(0);
 /*$profile_sql=mysqli_query($con,"select student_name,email_id,contact_no from user1 where stu_id=".$_SESSION['stu_id']); 
		$profile_fetch_sql=mysqli_fetch_assoc($profile_sql);*/
...
<div id="container">
<div class="paypal">
<img src="http://www.skillizen.com/img/Logo.gif">
<p> Please Wait...... we are redirecting to paypal !! Don't Refresh or press back button</p>
<img src="http://www.skillizen.com/img/Secure.png">
</div>
</div>
<form name='frmPayPal1' id="frmPayPal1"  method="post" action='<?php echo $paypal_url; ?>'  style="margin-left:300px; margin-top:200px;">    
		...
		<input type='hidden' name='image_url' value='http://www.skillizen.com/img/Logo_pay.png'> 
        <!-- logo of your website -->
		<input type="hidden" name="rm" value="2" />    <!--1-get 0-get 2-POST -->
        <input type='hidden' class="name" name='item_name' value='<?php echo "OLYMPIAD"; ?>'>
  ...
  ?>">
        ...
        <input type='hidden' name='cancel_return' value='http://www.iskillsolympiad.org/cancel.php?pageid=cancel'>
        <input type='hidden' name='return' value='http://www.iskillsolympiad.org/success.php?pageid=success&customerid=<?php echo $customerid; ?>&order_id=<?php echo $orderid; ?>'>
		<input type="hidden" name="notify_url" value="http://www.iskillsolympiad.org/" /> 
...
<?php } 
	...
	elseif($_REQUEST[payment]=='payu'){  
	  $profile_sql=mysqli_query($con,"select Student_Name,Email,Phone_No from studentt where Email='$customerid'"); 
	  $profile_fetch_sql=mysqli_fetch_assoc($profile_sql);	
...
$posted['email']=$profile_fetch_sql['Email'];
 $posted['phone']=$profile_fetch_sql['Phone_No'];
 $posted['productinfo']='Olympiad';
$posted['surl']='http://www.iskillsolympiad.org/success_payu.php';
$posted['furl']='http://www.iskillsolympiad.org/failure.php';
